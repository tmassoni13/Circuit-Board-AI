async function captureBoardImageSet(options = {}) {
        ...
        appendTerminalLine(
          `[${logPrefix} PLAN] board=${settings.widthMm}x${settings.heightMm}mm fov=${settings.fovWidthMm}x${settings.fovHeightMm}mm step=${capturePlan.useFullFovStep ? `full FOV + ${capturePlan.largeBoardStepExtraMm}mm` : `${CAPTURE_OVERLAP_MM}mm overlap`} tiles=${capturePlan.columns}x${capturePlan.rows} images=${capturePlan.imagesPerBoard}`
        );
